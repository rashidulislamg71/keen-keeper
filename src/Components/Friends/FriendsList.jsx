            ...
            md:gap-10'>
                {
                    friends.map(friend => (
                       <Link to ={`/friendDetails/${friend.id}`} key={friend.id} >
                        <div className='hover:shadow-xl transition-all duration-300  text-center m-auto h-auto w-40 md:w-55 lg:w-60 flex flex-col justify-center items-center shadow gap-2 bg-white border border-gray-300 p-5 rounded ' >
                            <img className='rounded-full w-20 h-20 ' src={friend.picture} alt={friend.name} />
